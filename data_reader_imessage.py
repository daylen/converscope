...
def read_imessage(path):
    ...
    for chat_id in chat_ids:
        ...
        messages = c2.execute(
            ...
            + str(chat_id[0]) +
            ") AND messageT.ROWID=chatMessageT.message_id LEFT JOIN handle handleT ON handleT.ROWID = messageT.handle_id ORDER BY messageT.date"
        )
        participants = set()
        for msg in messages:
            if msg[0] is None:
        ...
        if len(conversation.participant) == 2:
            conversation.group_name = (conversation.participant[0] if
                                       conversation.participant[0] != SELF_NAME
                                       else conversation.participant[1])
...
def main():
    ...
    f.close()
    print('Wrote', len(inbox.conversation), 'conversations to',
          EXPORT_PATH_IMESSAGE)
