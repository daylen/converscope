...
def read_imessage(path):
	...
	for chat_id in chat_ids:
		...
		messages = c2.execute("SELECT text, handle_id, date, is_from_me FROM message messageT INNER JOIN chat_message_join chatMessageT ON (chatMessageT.chat_id=" + str(chat_id[0]) + ") AND messageT.ROWID=chatMessageT.message_id ORDER BY messageT.date")
		for msg in messages:
			print('Message:',msg)
	conn.close()
