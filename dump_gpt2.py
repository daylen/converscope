...
def conv_to_string(conv, ts_range):
	in_range = filter(lambda x: x.timestamp >= ts_range[0] and x.timestamp < ts_range[1] and x.content_type == chat_pb2.Message.CT_TEXT, conv.message)
...
def dump_for_gpt2(ia, ts_range=(0, sys.maxsize), fname_prefix=''):
	...
	with open('./' + fname_prefix + 'test.txt', 'w') as f:
		f.write(test_txt)
	print(train_count + test_count, 'messages', (train_count)/float(train_count + test_count), ts_range, fname_prefix)
...
def main():
	...
	dump_for_gpt2(ia, ts_range=(TS_SPLIT, sys.maxsize), fname_prefix='nov2016_dec2019')
